...
-   hugo生成博客
    ...
    ```
    如果想要对所有的org文件都执行自动保存用于发布，可以参考[^fn:11]。
    ...
    1.  创建git仓库
        ...
        ```
        打开emacs执行 `C-x C-f ~/notes/.github/workflows/main.yml RET` ，把下面的内容[^fn:12]粘贴进去，按 `C-x C-s` 保存
        ```nil
        name: github pages
...
这块内容主要参考了这几篇文章[^fn:13]<sup>, </sup>[^fn:2]

-   修改node名字
    修改TITLE，就能更改node名字，如果要解决文件名不一致的问题，再修改文件名[^fn:14]
-   代码块自动生成
...
-   rss
    ...
    ```
    elfeed-org用于管理rss源[^fn:16]，但是安装出现了一些问题
...
-   进度管理
    为了对不同的项目进度进行概览，准备试一下org-mode的任务管理用法，同时记录一些快捷键，参考了这两篇教程[^fn:17] <sup>, </sup>[^fn:18]
    ```nil
...
[^fn:11]: <https://ox-hugo.scripter.co/doc/hugo-section/>
[^fn:12]: <https://gohugo.io/hosting-and-deployment/hosting-on-github/>
[^fn:13]: <http://holbrook.github.io/2012/04/12/emacs_orgmode_editor.html>
[^fn:14]: <https://org-roam.discourse.group/t/how-to-rename-a-note-with-everything-updated-at-the-same-time/300/12>
[^fn:15]: <https://mp.ofweek.com/ic/a556714487227>
[^fn:16]: <https://zhuanlan.zhihu.com/p/556496150>
[^fn:17]: <http://holbrook.github.io/2012/04/14/emacs_orgmode_task.html>
[^fn:18]: <https://www.cnblogs.com/quantumman/p/10808374.html>
[^fn:19]: <https://orgmode.org/manual/Using-column-view.html>